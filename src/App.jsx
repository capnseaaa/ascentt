...
function computeHints(club, matchday, seenOneTimeHints, recentForm, tier) {
  ...
  if (hints.length < 2) {
    pushOnce("all-clear", "Squad and finances look solid — a good time to check the Draft or Market for a squad upgrade before your rivals do.");
  }
...
function ownershipDepositFor(tierIdx, difficulty, club, tierClubs) {
  ...
  let multiplier = Math.max(0.9, 1 + wealthBonus * 0.45 + noise);
  // A real handful of clubs (the actual richest ownership groups) sit well
  // above even the rest of the "rich" tier — Man City/Chelsea-style money,
  // not just "a good, well-run club." Ranked by reputation within the
  // tier's own clubs (ties broken by id, so it's deterministic and never
  // more than the intended few), capped at the top 3. Toned down from an
  // earlier pass — playtester feedback that Premier League money felt too
  // high overall once both this and the wealth bonus above stacked up.
...
function ClubSelectScreen({ world, onPick, saveWasReset, difficulty, onBack, defaultCountry, managerReputation, isJobSearch, onResetToNewSave }) {
  ...
  const requiredRepFor = (club, tierMin, tierMax) => {
    ...
    return Math.round(35 + percentile * 55);
  };
          ...
          {isJobSearch && onResetToNewSave && (
            confirmingReset ? (
              <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button onClick={onResetToNewSave} style={{ background: "none", border: `1px solid ${PALETTE.crimson}`, color: PALETTE.crimson, fontSize: 11, cursor: "pointer", ...display, padding: "4px 8px", borderRadius: 5 }}>
                  Yes, reset
                </button>
...
function TrophyTab({ trophyLog, bestFinish, bestFinishUsa, bestFinishEngland, currentClubName, worldRecords }) {
  const ordinal = (n) => {
    const s = ["th", "st", "nd", "rd"];
    const v = n % 100;
  ...
  // Season markers exist purely for counting seasons at a club — never
  // shown in the chronological log itself.
  ...
  // Trophy summary — one box PER CLUB the manager has ever been at in this
  // career, not just the current one (a title won three clubs ago used to
  // just vanish from this view the moment you moved on). Current club
  // first, then every other club in the order they first appear.
...
function computeInboxUrgentCount(club, matchday, tier, seenOneTimeHints, difficulty) {
  ...
  return (hints[0]?.id === "all-clear" ? 0 : hints.length) + (hasBoardMessage ? 1 : 0);
}
...
function InboxTab({ club, matchday, tier, managerHistory, setManagerHistory, difficulty }) {
  const seenOneTimeHints = managerHistory?.seenOneTimeHints || [];
  const recentForm = tier ? (computeTable(tier).find((r) => r.clubId === club.id)?.form.slice(-5) ?? []) : [];
  const hints = computeHints(club, matchday, seenOneTimeHints, recentForm, tier);
  const hasBoardMessage = DIFFICULTY_MODES[difficulty]?.boardMessages && club.boardMessage;

  // Mark one-time hints as seen once the tab has actually been viewed —
  // a short delay so the hints that are about to disappear are readable
  // for a moment first, mirroring the old "mark on close" behavior from
  // when this was a popup.
  useEffect(() => {
    const oneTimeIds = hints.filter((h) => h.oneTime).map((h) => h.id);
    if (oneTimeIds.length === 0 || !setManagerHistory) return;
  ...
  return (
    <div>
      ...
      {hints.map((h, i) => (
        <div key={h.id} style={{ fontSize: 13, ...serif, color: PALETTE.ink, padding: "10px 4px", borderBottom: i < hints.length - 1 ? `1px solid ${PALETTE.parchmentDim}` : "none" }}>
          {h.text}
        </div>
      ))}
...
function updateWorldRecordsFromMatch(next, result, seasonNumber) {
...
function checkCareerGoalsRecord(next, scorer, clubName, seasonNumber) {
  if (!next.worldRecords) next.worldRecords = { ...DEFAULT_WORLD_RECORDS };
  if (!next.newsFeed) next.newsFeed = [];
...
function checkTransferRecord(next, playerName, fee, fromClub, toClub, seasonNumber) {
  if (!next.worldRecords) next.worldRecords = { ...DEFAULT_WORLD_RECORDS };
  if (!next.newsFeed) next.newsFeed = [];
...
function Dashboard({ state, setState, onNewGame, onSacked, onLeaveClub, managerHistory, setManagerHistory }) {
  ...
  const handleBuy = (playerId, sellerId, sellerTierId) => {
    ...
    mutateAndSave((next) => {
      ...
      checkTransferRecord(next, p.name, p.askingPrice, seller.name, buyer.name, next.seasonNumber);
    ...
    <div style={{ minHeight: "100vh", background: PALETTE.parchment, ...serif }}>
      ...
      <div style={{ display: "flex", gap: 4, padding: "10px 24px 0", borderBottom: `2px solid ${PALETTE.parchmentDim}`, flexWrap: "wrap" }}>
        {TABS.map((t) => {
          ...
          const inboxCount = t.id === "inbox" ? computeInboxUrgentCount(userClub, currentMatchday ?? (state.seasonNumber > 1 ? 999 : 1), tier, managerHistory?.seenOneTimeHints || [], state.difficulty) : 0;
          return (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
      ...
      <div style={{ padding: 24, maxWidth: 900, margin: "0 auto" }}>
        ...
        {tab === "inbox" && <InboxTab club={userClub} matchday={currentMatchday ?? (state.seasonNumber > 1 ? 999 : 1)} tier={tier} managerHistory={managerHistory} setManagerHistory={setManagerHistory} difficulty={state.difficulty} />}
...
const DEFAULT_MANAGER_HISTORY = { trophyLog: [], bestFinish: null, bestFinishUsa: null, bestFinishEngland: null, hasSeenTutorial: false, seenOneTimeHints: [], managerReputation: 50 };
...
export default function App() {
  ...
  if (isPickingNewClub) {
    ...
    return <ClubSelectScreen world={state.tiers} defaultCountry={pendingCountry} saveWasReset={false} difficulty={pendingDifficulty} managerReputation={managerHistory.managerReputation} isJobSearch={isJobSearch} onBack={() => setPendingCountry(null)} onResetToNewSave={handleNewGame} onPick={(tierId, clubId) => {
  ...
  if (!state) {
    ...
    return <ClubSelectScreen world={previewWorld} defaultCountry={pendingCountry} saveWasReset={saveWasReset} difficulty={pendingDifficulty} managerReputation={managerHistory.managerReputation} isJobSearch={isJobSearch} onBack={() => setPendingCountry(null)} onResetToNewSave={handleNewGame} onPick={(tierId, clubId) => {
